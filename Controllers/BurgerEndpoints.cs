...
namespace APIBurger.Controllers;
...
public static class BurgerEndpoints
{
    public static void MapBurgerEndpoints (this IEndpointRouteBuilder routes)
    {
        ...
        group.MapGet("/", async (HodbContextContext db) =>
        {
        ...
        group.MapGet("/{id}", async Task<Results<Ok<Burger>, NotFound>> (int burgerid, HodbContextContext db) =>
        {
        ...
        group.MapPut("/{id}", async Task<Results<Ok, NotFound>> (int burgerid, Burger burger, HodbContextContext db) =>
        {
        ...
        group.MapPost("/", async (Burger burger, HodbContextContext db) =>
        {
        ...
        group.MapDelete("/{id}", async Task<Results<Ok, NotFound>> (int burgerid, HodbContextContext db) =>
        {
            var affected = await db.Burgers
                .Where(model => model.BurgerId == burgerid)
                .ExecuteDeleteAsync();
